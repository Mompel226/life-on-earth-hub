...
/* ---------- the shared progress files ----------
   ...
   NOTHING SCHOOL-SPECIFIC IS COPIED. This hub is one repository, linked from both the NLCS and
   the open edition of the front door, so it must never carry a marks address or a Client ID. It
   shows only what a student's own browser remembers; signing in, and bringing handed-in work
   back from a teacher's spreadsheet, belongs to the front door, where each school sets its own
   address in js/local.js. */
